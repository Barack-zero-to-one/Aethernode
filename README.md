...
| `--key-passphrase-env ENV_VAR` (any `client.py` command) | Encrypt/decrypt your identity key at rest using a passphrase from the named environment variable, prompting interactively if unset (opt-in; see Hardening) |
...
| `--relay-key-passphrase-env ENV_VAR` | Same as `--key-passphrase-env`, for the relay's own gossip identity key (see Hardening) |
| `--max-concurrent-connections` / `--gossip-max-concurrent-connections` | Cap on concurrent in-flight connections before they're rejected outright, for the client-facing and peer-facing listeners respectively (defaults 200/100; see Hardening) |
...
| Deletion anti-entropy | A relay's periodic anti-entropy pass also reconciles confirmed deletions it's missing from each trusted peer, the same way it already reconciles messages, so a delete surviving only as a single, best-effort push that happened to fail no longer strands a message on a peer that should have removed it |
| Encrypted key storage (opt-in) | `--key-passphrase-env`/`--relay-key-passphrase-env` encrypt the identity key file at rest via `cryptography`'s `BestAvailableEncryption`; unencrypted by default to keep this a low-friction CLI tool, since every invocation is a fresh process with no session to cache a passphrase across |
| Connection-flood cap | Both the client-facing and peer-facing (mTLS) listeners bound concurrent in-flight connections with a semaphore, acquired before a worker thread is even spawned and covering the entire connection lifetime including the gossip listener's handshake, closing an unauthenticated thread/file-descriptor exhaustion vector rather than just rate-limiting after acceptance |
...
Mesh-wide deletion originally relied entirely on that one push succeeding; if it failed for a peer that already held the message — a brief network blip, that peer restarting mid-fan-out — nothing else ever told it the message had been deleted. A relay's periodic anti-entropy pass now reconciles deletions the same way it already reconciles messages: alongside pulling new messages from each trusted peer, it also pulls confirmed tombstones that peer has and this relay doesn't (`GET /gossip/deletions?since_id=N`, mirroring `/gossip/messages` exactly), deletes the message locally if still held, and records the tombstone either way. No fresh authorization check is needed on a pulled tombstone — the peer already performed it before confirming — and because the pulled tombstone is written into this relay's own table, it becomes available to whichever of *this* relay's peers pull from it next, the same passive, transitive convergence that already lets messages propagate multiple hops without any relay needing to explicitly re-forward anything.
...
Two further hardening measures address concerns that a straightforward threat-model review of the relay surfaces once everything above is already in place, both scoped deliberately narrow rather than attempting a broader redesign this project's own scope doesn't call for (see below for what was considered and set aside).

Both `client.py`'s identity key and a relay's own gossip identity key were, until now, always written to disk as plain, unencrypted PEM files — `chmod 0600` where the platform supports it, and nothing at all otherwise. Anyone with filesystem read access to that file — malware, another user on a shared machine, a leaked backup — recovered the complete private key with no further barrier. `--key-passphrase-env ENV_VAR` (on any `client.py` command) and `--relay-key-passphrase-env ENV_VAR` (on `relay.py`) name an environment variable holding a passphrase; `cryptography`'s `BestAvailableEncryption` is used in place of `NoEncryption()` for the on-disk key, with an interactive, unechoed prompt if the named variable happens to be unset at the moment a passphrase is actually needed. Passphrases are always resolved through an environment variable name or an interactive prompt, never accepted as a literal command-line argument, since a literal value would leak into shell history and process listings. This stays opt-in rather than becoming the default: a CLI tool re-invoked as a fresh process on every single command has no session or agent to cache a passphrase across, so requiring one on every invocation would be a real usability cost for what has otherwise stayed a low-friction tool, consistent with `--delete-after-read` and the `direct` gossip transport already being opt-in for the same reason. Every key this project has ever generated still loads exactly as before; an existing unencrypted key is detected automatically and never demands a passphrase it doesn't need. An operator running a relay unattended (systemd, a process manager) needs the passphrase environment variable set in that service's own environment for automatic restarts to keep working, since an interactive prompt would hang a non-interactive service start.

Separately, both the client-facing and the peer-facing gossip listener previously accepted an unbounded number of concurrent connections — `socketserver.ThreadingMixIn`'s stock behavior spawns a new thread per accepted connection with no cap at all, and for the gossip listener specifically, the expensive part (the mTLS handshake and certificate verification) happens inside that thread, after acceptance but before any authentication has succeeded. A connection flood from a source that never authenticates at all could exhaust threads and file descriptors. `--max-concurrent-connections` (client-facing, default 200) and `--gossip-max-concurrent-connections` (peer-facing, default 100, smaller because a trusted-peer mesh is typically smaller and each connection there costs more) bound this with a `threading.BoundedSemaphore`, acquired non-blocking before a worker thread is even spawned and released only once that connection's entire lifetime — handshake included — has completed; a connection arriving at the cap is closed immediately rather than queued, so the exhaustion point is actually closed rather than merely moved.

Three other angles came up in the same review and were deliberately left alone, because closing any of them properly would require a materially larger redesign this project's own scope doesn't justify: replacing the static-RSA scheme with a full ratchet protocol for forward secrecy; a multi-hop, onion-routed redesign of gossip itself so no single relay in a trusted mesh ever sees full routing metadata; and mixnet-style batched delivery against a global passive adversary correlating publish and fetch timing, which even Tor itself does not fully solve. Full at-rest database encryption (a SQLCipher-style engine, or a RAM-only mode with aggressive eviction) was also considered and set aside for the same reason — the added dependency weight and platform-packaging risk of a non-stdlib SQLite engine isn't justified for a reference implementation that isn't aimed at mass distribution.
...
AetherNode is a reference implementation of the protocol rather than a hardened, production-ready deployment. A production deployment would additionally benefit from a retry queue for gossip pushes that fail on the first attempt rather than relying solely on the next anti-entropy pass to recover them. See Hardening for the three concrete attack angles addressed there, and the three broader ones (forward secrecy, a multi-hop redesign of gossip routing, and mixnet-style timing protection) deliberately left open as out of this project's actual scope.

This release adds passphrase-encrypted identity keys, a connection-flood cap on both listeners, and anti-entropy reconciliation for deletions, on top of earlier breaking changes: rotating, HMAC-based recipient addressing; cryptographic TTL enforcement, per-recipient storage quotas, and recipient-authorized secure deletion; a relay no longer reachable over plain TCP/IP under any circumstance; padded message payloads; and a second socket for mutually-authenticated gossip between relays. The `deletion_requests` table gains an `id INTEGER PRIMARY KEY AUTOINCREMENT` column to support the same cursor-based pagination `messages` already uses for anti-entropy, and `target_signature` becomes a `UNIQUE` indexed column rather than the primary key — every existing lookup was already `WHERE target_signature = ?`, so this is purely a schema shape change, no query rewrites. Anyone upgrading from an earlier version should delete any existing `aether.db`, `aether-relay.sock`, and `aether-relay-gossip.sock`, and reconfigure Tor according to the Deployment section above, before running this version — messages addressed under the old static recipient-addressing scheme will never be recomputed by any client again and are permanently unfetchable regardless.
