...
`stress_test_quota_multiday.py` demonstrates the per-day amplification described above with the same rigor: it launches a relay configured with a short, five-day retention window, forges every one of those five days' rotating identifiers from a single victim's public key alone, and floods each one in turn. The predicted worst case (`quota × retention_days`) and the realized total matched exactly, reached in well under a minute of wall-clock time rather than the five real days the retention window nominally spans, which is the concrete shape of the tradeoff described above rather than an abstract warning.
